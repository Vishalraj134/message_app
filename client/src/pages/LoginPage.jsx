...
const LoginPage = () => {
  ...
  return (
    ...
    <form onSubmit={onSubmitHandler} className='border-2 bg-white/8 text-white border-gray-500 p-6 flex flex-col gap-6 rounded-lg shadow-lg'>
      <h2 className='font-medium text-2xl flex justify-between item-center'>
        {currState}
      ...
      {currState==="Sign up" && !isDataSubmitted &&(
        ...
        type="text" className='p-2 border border-gray-500 rounded-md focus:outline-none' placeholder="Full Name" requested/>
      )}
      ...
      <div className='flex flex-col gap-2'>
        {currState==="Sign up"?(
          ...
          className='front-medium text-violet-500 cursor-pointer'>Login here</span></p>
        ):(
          <p className='text-sm text-gray-600'>Create an account <span 
          onClick={()=>setCurrState("Sign up")}
          className='front-medium text-violet-500 cursor-pointer'>Click here</span></p>
        )}
      </div>
